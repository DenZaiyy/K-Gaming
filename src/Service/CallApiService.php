...
class CallApiService extends AbstractController
{
    ...
    public function getCoverByGame ($game): array
    {
        ...
        return $this->connectAPI(// Requête à effectuer pour récupérer la cover du jeu remplaçant le nom du jeu par la variable $game
    ...
    public function connectAPI ($data): array
    {
        ...
        $response = $this->client->request(// Méthode HTTP utilisée
          ...
          [// En-têtes de la requête (obligatoire)
            "headers" => [// Client-ID et Authorization sont des en-têtes obligatoires pour se connecter à l'API IGDB
    ...
    {
        // Utilisation de la fonction connectAPI pour se connecter à l'API IGDB
        return $this->connectAPI(// Requête à effectuer pour récupérer le résumé du jeu remplaçant le nom du jeu par la variable $game
    ...
    {
        return $this->connectAPI('fields name, screenshots.image_id; where name = "' . $game . '";');
    }
    ...
    {
        return $this->connectAPI('fields name, videos.name, videos.video_id; where name = "' . $game . '";');
    }
}
