...
class CartService extends AbstractController
{
  ...
  public function addToCart($gameSlug, $platformSlug): void
  {
    ...
    foreach ($cart as $key => $item) {
      ...
      if (
        $item["game"]->getId() == $game->getId() &&
        $item["platform"]->getId() == $platform->getId()
      ) {
        $found = $key;
        break;
      }
    ...
    if ($found !== null) {
      if ($cart[$found]["quantity"] < $gameStock["total"]) {
        ...
        $flashType = "danger";
        $flashMessage =
          "Vous ne pouvez pas ajouter plus de jeux à votre panier";
  ...
  public function removeToCart($gameSlug, $platformSlug): void
  {
    ...
    foreach ($cart as $key => $item) {
      ...
      if (
        $item["game"]->getSlug() === $gameSlug &&
        $item["platform"]->getSlug() === $platformSlug
      ) {
