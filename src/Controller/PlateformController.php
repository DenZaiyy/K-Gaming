...
class PlateformController extends AbstractController
{
  ...
  public function index(
    ...
    $gamesAvailable = $em
      ...
      ->findGamesInPlatform($platform->getId()); // Récupération des jeux disponibles dans la plateforme

    $games = $em
      ->getRepository(Game::class)
      ->findSearch($data, $platform, $resultPerPage); // Récupération des jeux en fonction des filtres de recherche
    [$min, $max] = $em
      ->getRepository(Game::class)
      ->findMinMax($data, $platform); // Récupération du prix minimum et maximum des jeux en fonction des filtres de recherche
    ...
    if ($request->get("ajax")) {
      return new JsonResponse([
        ...
        "pagination" => $this->renderView(
          "game/platform/_pagination.html.twig",
          ["games" => $games, "platform" => $platform]
        ),
        "pages" => ceil(
          $games->getTotalItemCount() / $games->getItemNumberPerPage()
        ),
        "min" => $min,
        "max" => $max,
      ]);
    ...
    return $this->render("game/platform/index.html.twig", [
      ...
      "max" => $max,
      "description" =>
        "Liste des jeux de la plateforme " .
        $platform->getLabel() .
  ...
  public function categories(
    ...
    return $this->render("game/platform/categories.html.twig", [
      ...
      "category" => $category,
      "description" =>
        "Liste des plateformes de la catégorie " .
        $category->getLabel() .
        " disponibles sur le site K-GAMING.",
    ]);
  }
