    ...
    public function subscribe (MailerInterface $mailer, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $email = (new TemplatedEmail())->from(
              new Address("support@k-grischko.fr", "K-GAMING - Newsletter")
            ) //set the sender
            ...
            $this->addFlash(
              "success", "Un email de confirmation vous a été envoyé à l'adresse suivante : " . $emailAddress
            );
            return $this->redirectToRoute("app_home");
        }

        return $this->render("newsletter/subscribe.html.twig", ["form" => $form->createView(),]);
    }
