...
class GameController extends AbstractController
{
  ...
  #[
    Route(
      "/{_locale<%app.supported_locales%>}/{gameSlug}",
      name: "app_show_game",
      priority: -1
    )
  ]
  public function showGame(
    ...
    $gameStock = $em
      ...
      ->findStockByGameID($game->getId());
    $gamePlatform = $em
      ->getRepository(Game::class)
      ->findOneGameInPlatform($game->getId(), $gameStock[0]["platform_id"]);
    $platform = $em
      ->getRepository(Plateform::class)
      ->findOneBy(["id" => $gamePlatform["platform_id"]]);
    ...
    return $this->render("game/show.html.twig", [
      ...
      "description" =>
        "Retrouvez toutes les informations concernant le jeu " .
        $game->getLabel() .
        " sur K-Gaming.",
    ]);
  }
  ...
  #[
    Route(
      "/{_locale<%app.supported_locales%>}/platform/{categoryLabel}/{platformSlug}/{gameSlug}",
      name: "app_show_game_platform"
    )
  ]
  public function showGameInPlatform(
    ...
    $gamePlatform = $em
      ->getRepository(Game::class)
      ->findOneGameInPlatform($game->getId(), $platform->getId());
    $gameStock = $em
      ->getRepository(Stock::class)
      ->findAvailableGameStockByPlatform($game->getId(), $platform->getId());
  ...
  #[
    Route(
      path: [
        "fr" => "/fr/jeux/precommande",
        "en" => "/en/preorder/game",
      ],
      name: "app_show_preorders"
    )
  ]
  public function showGameInPreorder(EntityManagerInterface $em): Response
  {
    ...
    return $this->render("game/preOrder/index.html.twig", [
      ...
      "description" =>
        "Retrouvez toutes les informations concernant les jeux en précommande sur K-Gaming.",
    ]);
  }
  ...
  #[
    Route(
      "/{_locale<%app.supported_locales%>}/game/genre/{genreSlug}",
      name: "app_show_game_genre"
    )
  ]
  public function showGameByGenre(
    ...
    $genre = $em
      ...
      ->findOneBy(["slug" => $genreSlug]);

    // Pagination KNPPaginator
    $pagination = $paginator->paginate(
      $em
        ->getRepository(Game::class)
        ->findGameByGenrePagination($genre->getSlug()),
    ...
    return $this->render("game/genre/show.html.twig", [
      ...
      "genre" => $genre,
      "description" =>
        "Retrouvez toutes les informations concernant les jeux du genre " .
        $genre->getLabel() .
        " sur K-Gaming.",
    ]);
  }
