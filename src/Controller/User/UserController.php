...
class UserController extends AbstractController
{
    ...
    public function index (Request $request, UserPasswordHasherInterface $hasher, MultiAvatars $multiAvatars): Response
    {
        ...
        if ($usernameForm->isSubmitted() && $usernameForm->isValid()) {
            $check = $this->em->getRepository(User::class)->findOneBy(
              ["username" => $usernameForm->getData()->getUsername()]
            );
            if ($check) {
        ...
        if ($emailForm->isSubmitted() && $emailForm->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation(
              "app_verify_email", $currentUser, (new TemplatedEmail())->from(
              new \Symfony\Component\Mime\Address("support@k-grischko.fr", "K-Gaming - Support")
            )->to($currentUser->getEmail())->subject("Confirmer votre nouvelle adresse email")->htmlTemplate(
              "security/user/user_email_update.html.twig"
            )
            );

            $this->addFlash(
              "success", "Votre adresse email a bien été modifié, vous allez recevoir un email de confirmation"
            );
            return $this->redirectToRoute("user_my_account");
        }
        ...
        if ($passwordForm->isSubmitted() && $passwordForm->isValid()) {
            if ($hasher->isPasswordValid($currentUser, $passwordForm->get("currentPassword")->getData())) {
                ...
                $currentUser->setPassword(
                  $hasher->hashPassword($currentUser, $passwordForm->get("plainPassword")->getData())
                );
        ...
        return $this->render("security/user/index.html.twig", ["user" => $user,
          "avatars" => $avatars,
    ...
    {
        $purchases = $this->em->getRepository(Purchase::class)->findBy(["user" => $this->getUser()],
          ["created_at" => "DESC"]);

        return $this->render("security/user/my-order.html.twig", ["purchases" => $purchases,
          "user" => $this->getUser(),
    ...
    public function myPreference (): Response
    {
        ...
        return $this->render("security/user/my-preference.html.twig", ["newsletter" => $newsletter,
    ...
    public function addAddress (Request $request): Response
    {
        ...
        return $this->render("security/user/address/add.html.twig", ["formAddAddress" => $form->createView(),
          "edit" => false,
          "description" => null,]);
    }
