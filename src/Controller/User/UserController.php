...
class UserController extends AbstractController
{
  ...
  public function index(
    ...
    if ($usernameForm->isSubmitted() && $usernameForm->isValid()) {
      $check = $this->em
        ->getRepository(User::class)
        ->findOneBy(["username" => $usernameForm->getData()->getUsername()]);
      if ($check) {
    ...
    if ($emailForm->isSubmitted() && $emailForm->isValid()) {
      $check = $this->em
        ->getRepository(User::class)
        ->findOneBy(["email" => $emailForm->getData()->getEmail()]);
      if ($check) {
      ...
      $this->addFlash(
        "success",
        "Votre adresse email a bien été modifié, vous allez recevoir un email de confirmation"
      );
      return $this->redirectToRoute("user_my_account");
    }
    ...
    return $this->render("security/user/index.html.twig", [
      ...
      "description" =>
        "Accéder à votre compte et modifier vos informations personnelles",
    ]);
  }
  ...
  public function myOrder(): Response
  {
    ...
    return $this->render("security/user/my-order.html.twig", [
      ...
      "user" => $this->getUser(),
      "description" =>
        "Accéder à l'historique de vos commandes passées sur K-Gaming",
    ]);
  }
  ...
  public function myPreference(): Response
  {
    $user = $this->getUser();
    $newsletter = $this->em
      ->getRepository(NewsletterUser::class)
      ->findOneBy(["email" => $user->getEmail()]);
    return $this->render("security/user/my-preference.html.twig", [
      "newsletter" => $newsletter,
      "description" =>
        "Accéder à vos préférences pour la newsletter et le thème du site K-Gaming",
    ]);
  }
