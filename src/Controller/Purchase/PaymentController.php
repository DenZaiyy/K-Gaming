...
class PaymentController extends AbstractController
{
  ...
  public function index($reference): RedirectResponse
  {
    ...
    $purchase = $this->em
      ...
      ->findOneBy(["reference" => $reference, "method" => "stripe"]); //get the purchase by the reference

    if (!$purchase) {
      //if the purchase doesn't exist, redirect to the cart
      $this->addFlash(
        "danger",
        "La commande avec la référence $reference n'existe pas"
      );
    ...
    foreach ($purchase->getRecapDetails()->getValues() as $product) {
      ...
      $productData = $this->em
        ...
        ->findOneBy(["label" => $product->getGameLabel()]); //get the game by the label
  ...
  public function purchaseSuccess(
    ...
    foreach ($recapDetails as $recapDetail) {
      ...
      $gameStockID = $this->em
        ...
        ->findLicenseKeyAvailableByGamesAndPlatform(
    ...
    $email = (new TemplatedEmail())
      ...
      ->from(
        new Address(
          ...
          "K-GAMING - Confirmation de commande"
        )
      )
  ...
  public function purchaseError($reference, CartService $cartService): Response
  {
    $purchase = $this->em
      ->getRepository(Purchase::class)
      ->findOneBy(["reference" => $reference]); //get the purchase by the reference
  ...
  public function createSessionPaypal($reference): RedirectResponse
  {
    ...
    $purchase = $this->em
      ->getRepository(Purchase::class)
      ->findOneBy(["reference" => $reference, "method" => "paypal"]);
    //Si la commande n'existe pas ou que la méthode n'est pas paypal, on redirige vers le panier
    if (!$purchase) {
      $this->addFlash(
        "danger",
        "La commande avec la reference $reference n'existe pas"
      );
      //if the purchase doesn't exist, redirect to the cart
      return $this->redirectToRoute("cart_index");
    }
