...
class PaymentController extends AbstractController
{
    ...
    public function index ($reference): RedirectResponse
    {
        ...
        $purchase = $this->em->getRepository(Purchase::class)->findOneBy(["reference" => $reference,
          "method" => "stripe"]); //get the purchase by the reference
        ...
        $checkout_session = Session::create(["customer_email" => $this->getUser()->getEmail(), //get the user email
          "payment_method_types" => ["card"],
          "line_items" => [[$productStripe], //add the products to the session
          ],
          "locale" => "fr",
          "mode" => "payment",
          "success_url" => $this->urlGenerator->generate("payment_success", ["reference" => $purchase->getReference()],
            UrlGeneratorInterface::ABSOLUTE_URL),
          "cancel_url" => $this->urlGenerator->generate("payment_error", ["reference" => $purchase->getReference()],
            UrlGeneratorInterface::ABSOLUTE_URL),
      ...
      PdfService $pdf
        ...
        foreach ($recapDetails as $recapDetail) {
            ...
            $gameStockID = $this->em->getRepository(Stock::class)->findLicenseKeyAvailableByGamesAndPlatform(
              $recapDetail->getGameId(), $recapDetail->getPlatformId(), $recapDetail->getQuantity()
            );
        ...
        foreach ($cartService->getTotal() as $item) {
            ...
            $product = ["game" => $item["game"], "platform" => $item["platform"], "quantity" => $item["quantity"],];
        ...
        $html = $this->render("order/invoice/facture.html.twig", ["user" => $user,
          "games" => $products,
          "purchase" => $purchase,
        ...
        $email = (new TemplatedEmail())
          ...
          ->addPart(
            new DataPart(
              new File($path . "/" . "facture-" . $idUniq . ".pdf"), "facture-" . $idUniq . ".pdf", "application/pdf"
            )
          );
        ...
        return $this->render("order/payment/success.html.twig", ["reference" => $reference,
          "products" => $products,
          "purchase" => $purchase,
          "description" => null,]);
    }
    ...
    {
        $purchase = $this->em->getRepository(Purchase::class)->findOneBy(["reference" => $reference]
        ); //get the purchase by the reference
        ...
        return $this->render("order/payment/error.html.twig", ["reference" => $reference, "description" => null,]);
    }
    ...
    public function createSessionPaypal ($reference): RedirectResponse
    {
        ...
        foreach ($purchase->getRecapDetails()->getValues() as $product) {
            ...
            $items[] = [//On récupère le nom du jeu
              "name" => $product->getGameLabel(), //On récupère la quantité
              "quantity" => $product->getQuantity(),
                //unit_amount est le prix unitaire du produit (prix du jeu) et currency_code est la devise (EUR)
              "unit_amount" => [//On récupère le prix du jeu
                "value" => $product->getPrice(), //On récupère la devise
                "currency_code" => "EUR",],];
        ...
        $request->body = [//On défini l'intent de la requête (CAPTURE)
          "intent" => "CAPTURE", //On défini les détails de la requête
          "purchase_units" => [[//On défini le montant de la requête
            "amount" => [//On défini la devise
              "currency_code" => "EUR", //On défini le prix total
              "value" => $total, //On défini le détail de la requête
              "breakdown" => [//On défini le prix total des produits
                "item_total" => [//On défini la devise
                  "currency_code" => "EUR", //On défini le prix total des produits
                  "value" => $itemTotal,], //On défini le prix de la taxe
                "shipping" => [//On défini la devise
                  "currency_code" => "EUR", //On défini le prix de la taxe
                  "value" => $shipping,],],], //On défini les produits de la requête récupérés dans la variable $items
            "items" => $items,],],
            //On défini le contexte de la requête (URL de redirection en cas de succès ou d'erreur)
          "application_context" => [//On défini l'URL de redirection en cas d'erreur
            "cancel_url" => $this->urlGenerator->generate(//On défini la route de redirection en cas d'erreur
              "payment_error", //On défini les paramètres de la route de redirection en cas d'erreur
              ["reference" => $purchase->getReference()], //On défini le type de redirection (URL absolue)
              UrlGeneratorInterface::ABSOLUTE_URL
            ), //On défini l'URL de redirection en cas de succès
            "return_url" => $this->urlGenerator->generate(//On défini la route de redirection en cas de succès
              "payment_success", //On défini les paramètres de la route de redirection en cas de succès
              ["reference" => $purchase->getReference()], //On défini le type de redirection (URL absolue)
              UrlGeneratorInterface::ABSOLUTE_URL
            ),],];
