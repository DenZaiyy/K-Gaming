...
class RatingController extends AbstractController
{
    ...
    public function index (EntityManagerInterface $em, Request $request, $gameSlug, $platformSlug): Response
    {
        ...
        $rating = $em->getRepository(Rating::class)->findOneBy(["user" => $user, "game" => $game,]);

        // Si l'utilisateur a déjà voté pour ce jeu, on le redirige vers la page d'accueil
        if ($rating) {
            $this->addFlash(
              "danger", "Vous avez déjà voter pour ce jeu, veuillez modifier votre note directement dans votre profil"
            );
        ...
        if ($ratingForm->isSubmitted() && $ratingForm->isValid()) {
            ...
            if ($ratingForm->getData()->getNote() == 0) {
                ...
                return $this->redirectToRoute("rating_game", ["gameSlug" => $gameSlug,
            ...
            $message = strip_tags(
              $ratingForm->getData()->getMessage()
            ); // Récupération du message et suppression des balises HTML avec strip_tags (pour éviter les failles XSS)
        ...
        return $this->render("rating/index.html.twig", ["ratingForm" => $ratingForm->createView(),
          "game" => $game,
          "platform" => $platform,
          "edit" => false,
          "ratingValue" => 0,
    ...
    public function edit (EntityManagerInterface $em, Request $request, $gameSlug, $platformSlug): Response
    {
        ...
        $rating = $em->getRepository(Rating::class)->findOneBy(["user" => $this->getUser(),
          "game" => $game,
          "platform" => $platform,]);
    ...
    public function ratingsList (EntityManagerInterface $em): Response
    {
        ...
        return $this->render("rating/list.html.twig", ["ratings" => $ratings,
