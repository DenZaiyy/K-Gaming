...
class PurchaseCrudController extends AbstractCrudController
{
  ...
  public function configureCrud(Crud $crud): Crud
  {
    return $crud
      ->setEntityLabelInSingular(
        fn(?Purchase $purchase, ?string $pageName) => $purchase
  ...
  public function configureFields(string $pageName): iterable
  {
    ...
    Crud::PAGE_EDIT === $pageName
      ? yield ArrayField::new("recapDetails")->setDisabled(true)
      : yield AssociationField::new("recapDetails", "Produit achetés")
        ->setDisabled(true)
        ->setSortable(false);
    yield FormField::addColumn(4);
    ...
    if (Crud::PAGE_EDIT === $pageName) {
      yield DateField::new("facture.created_at")
        ->setDisabled(true)
        ->setFormat("dd-MM-yyyy")
        ->setFormTypeOptions([
          "data" => new DateTime("now", new DateTimeZone("Europe/Paris")), // default data
