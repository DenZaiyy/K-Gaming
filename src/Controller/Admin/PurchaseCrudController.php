...
class PurchaseCrudController extends AbstractCrudController
{
    ...
    {
        return $crud
            ->setEntityLabelInSingular(fn (?Purchase $purchase, ?string $pageName) => $purchase ? $purchase->getReference() : 'Purchase')
    ...
    public function configureFields(string $pageName): iterable
    {
	    ...
	    yield FormField::addPanel("Pseudo de l'utilisateur");
			Crud::PAGE_EDIT === $pageName ?  yield TextField::new('user')->setDisabled(true) : yield AssociationField::new('user')->hideOnForm();
	    yield FormField::addColumn(4);
	    yield FormField::addPanel('Récapitulatif de la commande');
			Crud::PAGE_EDIT === $pageName ? yield ArrayField::new('recapDetails')->setDisabled(true) : yield AssociationField::new('recapDetails', 'Produit achetés')->setDisabled(true)->setSortable(false);
		yield FormField::addColumn(4);
