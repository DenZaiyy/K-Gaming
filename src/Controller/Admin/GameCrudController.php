...
class GameCrudController extends AbstractCrudController
{
    ...
    public function updateEntity (EntityManagerInterface $entityManager, $entityInstance): void
    {
        ...
        if ($originalEntityData["is_promotion"] === $entityInstance->isIsPromotion(
          ) && $originalEntityData["promo_percent"] === $entityInstance->getPromoPercent()) {
        ...
        if ($entityInstance->isIsPromotion()) {
            ...
            $entityInstance->setPrice(
              $entityInstance->getPrice() - ($entityInstance->getPrice() * $entityInstance->getPromoPercent()) / 100
            );
        } else {
    ...
    public function configureCrud (Crud $crud): Crud
    {
        return $crud->setEntityLabelInSingular(
          fn(?Game $product, ?string $pageName) => $product
            ? $product->getLabel() : new TranslatableMessage(
              "menu.game.singular", [], "admin"
            )
        )->setEntityLabelInPlural(new TranslatableMessage("game.index.title", [], "admin"))->setHelp(
          Crud::PAGE_INDEX, new TranslatableMessage("game.index.description", [], "admin")
        )->setSearchFields(["id",
          "label",
          "price",
          "is_promotion",
          "slug",
          "date_release",])->setEntityPermission("ROLE_ADMIN")->showEntityActionsInlined();
    }
    ...
    {
        return $filters->add("label")->add("price")->add("is_promotion")->add("is_sellable")->add("date_release")->add(
          EntityFilter::new("plateforms")
    ...
    public function configureFields (string $pageName): iterable
    {
        ...
        yield SlugField::new("slug", new TranslatableMessage("game.table.slug", [], "admin"))->setTargetFieldName(
          "label"
        );
        yield FormField::addColumn(4);
        yield FormField::addPanel(new TranslatableMessage("game.detail.status", [], "admin"))->setHelp(
        ...
        yield MoneyField::new("price", new TranslatableMessage("game.table.price", [], "admin"))->setCurrency(
          "EUR"
        )->setStoredAsCents(false);
        yield MoneyField::new("old_price", new TranslatableMessage("game.table.old_price", [], "admin"))->setCurrency(
          "EUR"
        )->setDisabled(true);
        yield FormField::addColumn(4);
        yield FormField::addPanel(new TranslatableMessage("game.detail.promotion", [], "admin"));
        yield BooleanField::new("is_promotion", new TranslatableMessage("game.table.is_promotion", [], "admin")
        )->setHelp(new TranslatableMessage("game.detail.promotion_description", [], "admin"));
        yield NumberField::new("promo_percent", new TranslatableMessage("game.table.promotion_percent", [], "admin")
        )->setHelp(new TranslatableMessage("game.detail.promotion_percent_description", [], "admin"));
        yield FormField::addTab(new TranslatableMessage("game.detail.game_information", [], "admin"));
        yield FormField::addColumn(4);
        yield FormField::addPanel()->setHelp(
          new TranslatableMessage("game.detail.game_platforms_description", [], "admin")
        );
        yield AssociationField::new("plateforms", new TranslatableMessage("game.table.platforms", [], "admin"));
        yield FormField::addColumn(4);
        yield FormField::addPanel()->setHelp(
          new TranslatableMessage("game.detail.game_genders_description", [], "admin")
        );
        ...
        )->setFormat("dd-MM-yyyy")->setFormTypeOptions(["data" => new DateTime("now", new DateTimeZone("Europe/Paris")),
            // default data
          "widget" => "single_text",
          "disabled" => true,]);
    }
