 ...
 */
export default class Filter { //export default permet d'exporter la classe pour pouvoir l'importer dans un autre fichier

    /**
     * @param {HTMLElement|null} element
     */
    constructor(element) { //constructeur de la classe
        if (element === null) { //si l'élément n'existe pas, on sort de la fonction
            return
        }
    ...
    async loadMore() { // loadMore permet de charger plus de résultats
    ...
    async loadForm() { // loadForm permet de charger les données du formulaire
        this.page = 1 //on remet la page à 1
        const data = new FormData(this.form) //on récupère les données du formulaire
        const url = new URL(this.form.getAttribute('action') || window.location.href) //on récupère l'url
        const params = new URLSearchParams() //construit les params d'une url dynamiquement

        data.forEach((value, key) => { //on boucle sur les données du formulaire
    ...
    async loadUrl(url, append = false) { // loadUrl permet de charger les données de l'url
        ...
        const response = await fetch(url.split('?')[0] + '?' + params.toString(), { //on appelle l'url avec les paramètres
            headers: { //on définit les headers
        ...
        if (response.status >= 200 && response.status < 300) {
            ...
            if(!this.moreNav) { //si on n'est pas sur la page d'accueil
                this.pagination.innerHTML = data.pagination //on affiche la pagination
            } else if (this.page === data.pages) { //si on est sur la dernière page
        ...
        } else { //sinon on affiche une erreur dans la console
     ...
     */
    flipContent(content, append = false) { // flipContent permet de remplacer les éléments de la grille avec un effet d'animation flip et d'ajouter les nouveaux éléments à la suite
        const springConfig = 'gentle' //on définit la config de l'animation
        const exitSpring = function (element, index, onComplete) { //on définit l'animation de sortie
            spring({ //on appelle la fonction spring de la librairie spring
                config: "stiff", //on définit la config de l'animation
                values: { //on définit les valeurs de l'animation
                    translateY: [0, -20],
                    opacity: [1, 0]
                },
                onUpdate: ({translateY, opacity}) => { //on définit les valeurs de l'animation
        ...
        const appearSpring = function (element, index) { //on définit l'animation d'apparition
            spring({ //on appelle la fonction spring de la librairie spring
                config: "stiff",
                values: {
                    translateY: [20, 0],
        ...
        const flipper = new Flipper({ //on appelle la fonction Flipper de la librairie flipjs
            element: this.content //on définit l'élément sur lequel on applique l'animation
        })

        // this.content.children.forEach(element => {
        Array.prototype.forEach.call(this.content.children, (element => { //on boucle sur les éléments de la grille pour appliquer l'animation de sortie sur les éléments existants avant de les supprimer de la grille et d'ajouter les nouveaux éléments
            flipper.addFlipped({
                element,
                spring: springConfig,
                flipId: element.id,
        ...
        if(append) { //si on veut ajouter les nouveaux éléments à la suite
        ...
        // this.content.children.forEach(element => {
        test.forEach(element => { //on boucle sur les nouveaux éléments pour appliquer l'animation d'apparition
            flipper.addFlipped({
                element,
                spring: springConfig,
                flipId: element.id,
...
showLoader() { //showLoader permet d'afficher le loader pendant le chargement des données de la requête ajax
        ...
        if(loader === null) { //si le loader n'existe pas, on sort de la fonction
            return
        }
    ...
    hideLoader() { //hideLoader permet de cacher le loader après le chargement des données de la requête ajax
    ...
    updatePrices({min, max}) { //updatePrices permet de mettre à jour les prix dans le slider après le chargement des données de la requête ajax pour que le slider s'adapte aux nouveaux prix des produits affichés
        ...
        slider.noUiSlider.updateOptions({ //on met à jour les options du slider
            range: {
